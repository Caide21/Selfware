...
import { ZONES } from '@/worlds/core/zones';
import { useZoneState } from '@/worlds/core/useZoneState';

// Extra, non-zone links (keep as-is for now).
const EXTRA_LINKS = [
...
export default function AppNav() {
  ...
  // Build nav links from zone registry so routes don’t drift.
  const zoneLinks = ZONES.filter((z) => z.id !== 'settings').map((z) => ({
    href: z.path,
    label: z.label,
  }));

  const NAV_LINKS = [...EXTRA_LINKS, ...zoneLinks];
  const icon = (activeZone?.icon ?? '').trim();
  const label = (activeZone?.label ?? '').trim();

  // Treat placeholder strings like "habits", "quests" as not-icons.
  const iconLooksLikeWord = /^[a-z0-9 _-]+$/i.test(icon);
  const showIcon = Boolean(icon) && !iconLooksLikeWord && icon.toLowerCase() !== label.toLowerCase();
      ...
      <NavBar>
        <div className="flex items-center justify-between gap-4 sm:gap-6">
          <Link href="/home" className="flex items-center gap-2 text-lg font-semibold text-text">
            ...
            <span className="text-base font-semibold tracking-[0.25em] text-text sm:text-lg">
              Selfware
            </span>
          </Link>
              ...
              return (
                <Link
                  ...
                  className={[
                    baseLinkClass,
                    active
                      ? 'bg-slate-900/5 text-text'
                      : 'text-text/70 hover:text-text hover:bg-slate-900/5',
                  ].join(' ')}
                >
          ...
          <div className="flex flex-1 items-center justify-end gap-3 lg:flex-none">
            <span className="hidden items-center gap-2 rounded-full bg-slate-900/5 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.4em] text-text/60 lg:inline-flex">
              {showIcon ? <span className="text-xs">{icon}</span> : null}
              <span>{label}</span>
            </span>
            <AuthStatus />
          </div>
