...
type Habit = Record<string, any>;

export interface HabitCardProps {
  habit?: Habit;
  card?: Habit;
  isNew?: boolean;
  onSaved?: (payload: Habit, meta: any) => Promise<void> | void;
  onDiscard?: () => void;
}

function normalizeEdited(habit: Habit = {}) {
  return {
    title: habit.title ?? '',
...
export default function HabitCard({ habit: habitProp, card, isNew: isNewProp = false, onSaved, onDiscard }: HabitCardProps) {
  ...
  const [edited, setEdited] = useState<Habit>(() => normalizeEdited(initial));
  const [lastSaved, setLastSaved] = useState<Habit>(() => normalizeEdited(initial));
  ...
  async function handleSaveClick() {
    ...
    await onSaved?.(payload, { isNew, id: (initial as Habit).id });
