...
function logMaintenanceError(label, error) {
...
function isMissingSourceNoteIdColumn(error) {
  const message = String(error?.message || '').toLowerCase();
  return error?.code === '42703' && message.includes('source_note_id');
}

async function insertMaintenanceCompletion(supabase, payload) {
  const { data, error } = await supabase
    .from(MAINTENANCE_COMPLETIONS_TABLE)
    .insert(payload)
    .select('*')
  ...
  if (payload.source_note_id && isMissingSourceNoteIdColumn(error)) {
    logMaintenanceError('Maintenance completion source_note_id unavailable; retrying without note link', error);
    ...
    logMaintenanceError('Maintenance completion fallback insert failed', fallbackError);
    throw fallbackError;
  }

  logMaintenanceError('Maintenance completion insert failed', error);
  throw error;
}

export async function completeMaintenanceLoop(supabase, { ownerId, id, title, sourceNoteId = null }) {
  ...
  if (isUuid(sourceNoteId)) completionPayload.source_note_id = sourceNoteId;

  const completion = await insertMaintenanceCompletion(supabase, completionPayload);
