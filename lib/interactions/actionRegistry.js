import { deleteNoteWithDerivedData } from '@/lib/notes/deleteNote';
...
async function runDeleteAction({ target, context, table, confirmMessage, onSuccess, errorLabel, deleteTarget }) {
  if (!target?.id) {
  ...
  try {
    if (deleteTarget) {
      await deleteTarget(context.supabase, target.id);
    } else {
    ...
    window.alert(`Could not ${errorLabel}. ${deleteError?.message || ''}`.trim());
  }
}
...
export async function runInteractionAction(actionKey, target, context = {}) {
  ...
  if (actionKey === INTERACTION_ACTIONS.deleteNote.key) {
    await runDeleteAction({
      ...
      deleteTarget: deleteNoteWithDerivedData,
    });
    return;
  }
